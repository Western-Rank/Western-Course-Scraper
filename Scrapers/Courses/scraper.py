...
def addToCSV(name, code, prereqs, antireqs, coreqs, precoreqs, prereqsLink, antireqsLink, coreqsLink, precoreqsLink, desc, location, extra):
    global courseIndex
    coursesCSV.loc[courseIndex] = {"course_name": name, "course_code": code, "prerequisites_text": prereqs, "antirequisites_text": antireqs, "corequisites_text": coreqs, "precorequisites_text": precoreqs, "prerequisites_link": prereqsLink,
                                   "antirequisites_link": antireqsLink, "corequisites_link": coreqsLink, "precorequisites_link": precoreqsLink, "description": desc, "location": location, "extra_info": extra}
    courseIndex += 1
...
def scrapeFromAcademicCalendar(startAt=0, endAt=len(catList)):
    ...
    # scraping logic
    start = time.time()
    for cat in catList[startAt:endAt]:
        print(cat)
        ...
        for panel in soup.find_all(class_="panel-default")[2:]:
            ...
            addToCSV(name=newTitle, code=code, prereqs=prereqs, antireqs=antireqs,
                     coreqs=coreqs, precoreqs=preAndCoreqs, prereqsLink=list(set(prereqsLink)), coreqsLink=list(set(coreqsLink)), antireqsLink=list(set(antireqsLink)), precoreqsLink=list(set(preAndCoreqsLink)), desc=desc, location=location, extra=extra)
    end = time.time()
    totTime = end-start
